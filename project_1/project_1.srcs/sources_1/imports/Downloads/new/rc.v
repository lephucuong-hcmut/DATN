module rc(
    ...
    generate
        ...
        for (i = 0; i < (25*`NUM_SUB_ROUNDS); i = i + 1) begin: generate_rc_const
            assign rc_const[i] = rc_const_all[(i >> `DIVCLOG2(`NUM_SUB_ROUNDS))*64 + (i & (`NUM_SUB_ROUNDS-1))*`PARALLEL_SLICES + `PARALLEL_SLICES-1 -: `PARALLEL_SLICES] ;
        end
    endgenerate

    always @(*) begin
        if(round < `MAX_ROUND_COUNT) round_natural = round + 1'b1;
        else round_natural = 0;
    end
