...
module hqc_rsdecod_elp #(
    ...
    output                  dout_valid_o,
    output  [7:0]           deg_sigma_o 
);
    genvar i;

    reg     [DIN_W-1:0]     syndrome_buf;
    reg     [DOUT_W-1:0]    sigma;
    wire    [119:0]         syndrome_swap;
    ...
    wire    [119:0]         gf_mul_in1, gf_mul_in2, gf_mul_out;
    reg     [5:0]           cnt;
    wire    [5:0]           mu;
    reg     [2:0]           cnt_en_buf;
    reg                     busy, dout_valid;
    ...
    always @(posedge clk_i)
        if(din_valid_i)
            ...
            syndrome_buf <= {syndrome_buf[7:0], syndrome_buf[239:8]};

    generate
        for (i=0; i < PARAM_DELTA; i=i+1) begin: swap
            assign syndrome_swap[8*i +:8] = syndrome_buf[239 - 8*i -:8];
        end
    endgenerate
    ...
    always @(posedge clk_i)
        ...
        else if(cnt_en_buf[1] & mask12) begin
            pp          <= mu;
            dp          <= d;
    ...
    always @(posedge clk_i)
        if(din_valid_i)
            ...
            X_sigma_p   <= mask12? {sigma[119:0], 8'd0} : {X_sigma_p[119:0], 8'd0};
    ...
    generate
        for (i=0; i < PARAM_DELTA; i=i+1) begin: gf_mul
            gfmul_00 MULT_ARRAY(
                .start (1'b1                ),
    ...
    assign gf_mul_in1 = cnt_en_buf[1]? {PARAM_DELTA{dd}} : sigma[127:8];
    assign gf_mul_in2 = cnt_en_buf[1]? X_sigma_p[127:8] : syndrome_swap;
    ...
    function [7:0] xor_byte(input [119:0] x);
        integer j;
        begin
            xor_byte = 0;
