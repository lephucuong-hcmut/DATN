...
module fft_part2 #(
    ...
    genvar ii;
    generate
        for(ii=0; ii<4; ii=ii+1) begin: ROUND
            fft_part2_round #(
                .ROUND          (ii ),
                .DIN_W          (DIN_W ),
                .DOUT_W         (DOUT_W)
...
module fft_part2_round #(
    ...
    reg     [7:0]   g; 
    wire    [7:0]   gf_out;
    reg             din_valid_d, busy;
    reg     [7:0]   cnt_in, cnt_out;
    reg [4+ROUND:0] cnt_in_d;
    ...
    syncfifo #(.DW(8), .AW(4+ROUND)) FIFO_FFT (
        .clk_i   (clk_i        ),
        .rst_ni  (rst_ni       ),
        .wr_i    (fifo_wr   ),
        .rd_i    (fifo_rd   ),
