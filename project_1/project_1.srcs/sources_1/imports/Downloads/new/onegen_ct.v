...
always@(posedge clk) begin
    if (rst) begin
        ...
        if (state == s_wait_for_init_mem) begin
        ...
        end else if (state == s_initialize) begin
            valid_s <= 1'b0;
            done_s <= 1'b0;
            addr_1 <= 0;
            addr_0 <= 0;
            ...
            state <= s_init_done;
            ready_s <= 1'b0;
        end else if (state == s_init_done) begin
         ...
         end else if (state == s_stall_for_ram) begin
         ...
         end else if (state == s_simulate_wait_start) begin
         ...
         end else if (state == s_wait_last_2) begin
             if (~collision_s) begin
         ...
         end else if (state == s_simulate_wait_start_last) begin
            if (count_sim_cycles < 8) begin
                state <= s_simulate_wait_start_last;
                count_sim_cycles <= count_sim_cycles + 1;
            end else begin
