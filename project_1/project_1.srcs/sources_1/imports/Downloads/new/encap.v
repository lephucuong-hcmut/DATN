...
module encap #( 
    ...
    parameter HASH_RAMDEPTH = 1 + (HASH_RAMBITS_32/32) + HASH_RAM_DOMSEP,   
    parameter HASH_LB_DOMSEP = 32'h80000290,
    parameter THETA_D_DOMSEP = 32'h80000088,
...
(
    ...
    assign encap_out = (encap_out_type == 0) ? {96'd0, shake_din_h} :
                       (encap_out_type == 1) ? {96'd0, d_out} : u_v_int;
    ...
    assign msg_addr = (m_wen)? m_addr : m_addr_int;
    ...
    always@(posedge clk) begin
        if (init_msg_in) begin
            msg_in <= 0;
        end else if (shift_msg_in) begin
    ...
    assign u_v_out_addr = u_v_out_en_int? u_v_out_addr_int: encap_out_addr;
    ...
    always@(posedge clk) begin
        ...
        end else if (block_capture) begin
    ...
    assign hash_addr = (hash_mem_wen)? hash_in_addr :
                       (encap_out_en && encap_out_type == 0)? encap_out_addr: h_addr;
    ...
    localparam s_load_m_in_size = 1;
    localparam s_load_m_out_size = 2;
    localparam s_load_m = 3;
    localparam s_load_last_m = 4;
    localparam s_comp_theta = 5;
    localparam s_save_theta = 6;
    localparam s_encrypt = 7;
    localparam s_wait_done_fw = 8;
    localparam s_update_hram_d = 9;
    localparam s_comp_d = 10;
    localparam s_save_d = 11;
    localparam s_save_last_d = 12;
    localparam s_wait_done_enc = 13;
    localparam s_load_u = 14;
    localparam s_save_u = 15;
    localparam s_load_dom_sep = 16;
    localparam s_load_v = 17;
    localparam s_save_v = 18;
    localparam s_load_dom_sep_v = 19;
    localparam s_stall_0 = 20;
    localparam s_append_05 = 21;
    localparam s_comp_ss = 22;
    localparam s_wait_ss = 23;
    localparam s_stall_1 = 24;
    localparam s_wait_for_resume_encap = 25;
    localparam s_done = 31;
    ...
    always@(posedge clk) begin
        if (rst) begin
            ...
            if (state == s_wait_start) begin
            ...
            end else if (state == s_load_m_out_size) begin
                done <= 1'b0;
                m_addr_int <= 0;
                theta_addr <= 0;
                ...
                sel_uv <= 0;
                fixed_weight_processing <= 0;
            end else if (state == s_load_m_in_size) begin
                done <= 1'b0;
                theta_addr <= 0;
                ...
                sel_uv <= 0;
                fixed_weight_processing <= 0;
            end else if (state == s_load_m) begin
                done <= 1'b0;
                theta_addr <= 0;
                shake_dout_ready_h <= 0;
                d_addr <= 0;
            ...
            end else if (state == s_load_last_m) begin
                state <= s_comp_theta;
                hash_in_addr <= hash_in_addr + 1;
                theta_addr <= 0;
                shake_dout_ready_h <= 0;
                ...
                sel_uv <= 0;
                fixed_weight_processing <= 0;
            end else if (state == s_comp_theta) begin
                state <= s_save_theta;
                theta_addr <= 0;
                ...
                sel_uv <= 0;
                fixed_weight_processing <= 0;
            end else if(state == s_save_theta) begin
                shake_dout_ready_h <= 1;
                ...
                state <= s_encrypt;
           end else if (state == s_encrypt) begin
                shake_dout_ready_h <= 0;
            ...
            end else if (state == s_wait_done_fw) begin
                shake_dout_ready_h <= 0;
                d_addr <= 0;
                u_v_out_addr_int <= 0;
                count_uv_blocks <= 0;
                ...
                if (done_fixed_weight) begin
            ...
            end else if (state == s_update_hram_d) begin
                    state <= s_comp_d;
                    shake_dout_ready_h <= 1;
                    d_addr <= 0;
                    u_v_out_addr_int <= 0;
                    ...
                    sel_uv <= 0;
                    fixed_weight_processing <= 0;
            end else if (state == s_comp_d) begin
                    state <= s_save_d;
                    shake_dout_ready_h <= 1;
                    d_addr <= 0;
                    u_v_out_addr_int <= 0;
                    ...
                    u_v_out_en_int <= 0;
                    sel_uv <= 0;
            end else if (state == s_save_d) begin
                shake_dout_ready_h <= 1;
                hash_in_addr <= 1;
            ...
            end else if (state == s_save_last_d) begin
                shake_dout_ready_h <= 1;
                u_v_out_addr_int <= 0;
                count_uv_blocks <= 0;
                u_v_out_en_int <= 0;
            ...
            end else if (state == s_wait_done_enc) begin
               shake_dout_ready_h <= 0;
               u_v_out_addr_int <= 0;
               count_uv_blocks <= 0;
               sel_uv <= 0;
            ...
            end else if (state == s_wait_for_resume_encap) begin
               shake_dout_ready_h <= 0;
               u_v_out_addr_int <= 0;
               count_uv_blocks <= 0;
               sel_uv <= 0;
            ...
            end else if (state == s_load_u) begin
               shake_dout_ready_h <= 0;
               u_v_out_en_int <= 1;
            ...
            end else if (state == s_save_u) begin
               shake_dout_ready_h <= 0;
               u_v_out_en_int <= 1;
               if (count_hash_inputs < HASH_M_U_32/32-1) begin
                   hash_in_addr <= hash_in_addr+1;
            ...
            end else if (state == s_load_dom_sep) begin
               shake_dout_ready_h <= 0;
               u_v_out_en_int <= 1;
               hash_in_addr <= hash_in_addr+1;
               count_hash_inputs <= count_hash_inputs + 1;
            ...
            end else if (state == s_stall_0) begin
                sel_uv <= 1;
                shake_dout_ready_h <= 0;
                u_v_out_en_int <= 1;
                state <= s_load_v;
                count_uv_blocks <= 0;
            end else if (state == s_load_v) begin
               shake_dout_ready_h <= 0;
               u_v_out_en_int <= 1;
            ...
            end else if (state == s_save_v) begin
               shake_dout_ready_h <= 0;
               u_v_out_en_int <= 1;
               if (count_hash_inputs <= HASH_RAMBITS_32/32 -1) begin
                   hash_in_addr <= hash_in_addr+1;
            ...
            end else if (state == s_load_dom_sep_v) begin
               shake_dout_ready_h <= 0;
               u_v_out_en_int <= 1;
               hash_in_addr <= hash_in_addr+1;
               count_hash_inputs <= count_hash_inputs + 1;
            ...
            end else if (state == s_append_05) begin
                state <= s_comp_ss;
            end else if (state == s_comp_ss) begin
                start_hash <= 1;
            end else if (state == s_wait_ss) begin
                shake_dout_ready_h <= 1;
        ...
        case(state)
            ...
            s_load_u: begin
                ...
                if (count_hash_inputs%34 == 0) begin
                    shake_in_type = 0;
                end else if (count_hash_inputs%34 != 0 && count_uv_blocks == 0) begin
                    shake_in_type = 3;
                end else begin 
                    shake_in_type = 2;
                end
            ...
            s_load_v: begin
                ...
                if (count_hash_inputs%34 == 0 && count_hash_inputs < HASH_RAMBITS_32/32 - 1) begin
                    block_capture = 0;
                end else if (count_hash_inputs >= HASH_RAMBITS_32/32 -1) begin
                    block_capture = 0;
                end else begin
                    block_capture = 1;
                end
                ...
                if (count_hash_inputs%34 == 0) begin
                    shake_in_type = 0;
                end else if (count_hash_inputs%34 != 0) begin
                    shake_in_type = 5;
                end else begin 
                    shake_in_type = 4;
                end
    ...
    reg [3:0] h_state = 0;
    localparam h_wait_start  = 0;
    localparam h_check_shake_ready = 1;
    localparam h_first_block = 2;
    localparam h_second_block = 3;
    localparam h_load_shake = 4;
    localparam h_stall = 5;
    ...
    always@(posedge clk) begin
         if (rst) begin
            ...
            end else if (h_state == h_check_shake_ready) begin
                hash_processing <= 1'b1;
                if (shake_din_ready_h) begin
                    h_state <= h_first_block;
                    h_addr <= h_addr + 1;
                end
            end else if (h_state == h_first_block) begin
               hash_processing <= 1'b1;
               h_addr <= h_addr + 1;
               h_state <= h_second_block;
            end else if (h_state == h_second_block) begin
               hash_processing <= 1'b1;
               h_state <= h_load_shake;
            end else if (h_state == h_load_shake) begin
            ...
            end else if (h_state == h_stall) begin
               hash_processing <= 1'b1;
               h_state <= h_load_shake;
               h_addr <= h_addr+1;
            end
