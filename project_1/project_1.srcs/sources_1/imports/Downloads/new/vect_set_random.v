...
localparam BLOCKS_IN_MEM_WIDTH = MEM_WIDTH/32;
localparam LOG_BIM = `CLOG2(BLOCKS_IN_MEM_WIDTH);
...
assign shake_din = (shake_input_type == 2'b01)? squeeze: 
...
assign rand_mem_in = (done == 1)? {{(MEM_WIDTH-N_32%MEM_WIDTH){1'b0}},rand_out_rearrange_reg[MEM_WIDTH-1:MEM_WIDTH - N_32%MEM_WIDTH]}: rand_out_rearrange;
...
reg [3:0] pk_state = 0;
localparam pk_wait_start  = 0;
localparam pk_load_rand = 1;
localparam pk_stall_0 = 2;
...
always@(posedge clk) begin
     if (rst) begin
        ...
        end else if (pk_state == pk_load_rand) begin
            done_reg <= 1'b0;
            shake_dout_ready <= 1'b1;
        ...
        end else if (pk_state == pk_stall_0) begin
            pk_state <= pk_wait_start;
            done_reg <= 1'b1;
            shake_dout_ready <= 1'b0;
        end
...
localparam s_init_shake = 0;
localparam s_wait_for_shake_out_ready = 1;
localparam s_shake_out_w = 2;
localparam s_shake_in_w = 3;
localparam s_load_new_seed = 4;
localparam s_stall_0 = 5;
reg [1:0] count_steps;
...
always@(posedge clk) begin
    if (rst) begin
        ...
        end else if (state_shake == s_shake_in_w) begin
        ...
        end else if (state_shake == s_load_new_seed) begin
            if (seed_addr == SEED_SIZE/32) begin
                state_shake <= s_shake_out_w;
            end else begin
                seed_addr <= seed_addr + 1;
            end
        end else if (state_shake == s_shake_out_w) begin
            if (count_steps == 1) begin
        ...
        end else if (state_shake == s_stall_0) begin
            if (done_reg) begin
                state_shake <= s_init_shake;
            end else begin
                state_shake <= s_stall_0;
