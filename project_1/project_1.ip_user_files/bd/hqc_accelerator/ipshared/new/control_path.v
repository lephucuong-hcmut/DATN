...
reg         [`COUNTER_WIDTH-1:0]      start_addr_reg = 0;

reg         [`COUNTER_WIDTH-1:0]      prev_start_addr = 0;
reg         [`COUNTER_WIDTH-1:0]      prev_start_addr_reg = 0;
...
begin: REGS_PROC
    if (rst) begin
        ...
        bof_internal_reg      <= 0;
        to_be_read_reg        <= 0;
        to_be_absorbed_reg    <= 0;
        data_length_reg       <= 0;
        cshake_reg            <= 0;
        extra_pad_reg         <= 0;
        requested_bytes_reg   <= 0;
        rate_reg              <= 0;
        max_reads_count_reg   <= 0;
        mux256_reg            <= 0;
        start_addr_reg        <= 0;
...
//always @ (current_state or next_state or din_valid or dout_ready or din 
//  or absorb_data or counter or bof_internal_reg or prev_start_addr_reg
//  or eof_internal_reg or to_be_read_reg or to_be_absorbed_reg or data_length_reg 
//  or requested_bytes_reg or cshake_reg or extra_pad_reg or din_save_reg 
//  or max_reads_count_reg or mux256_reg or rate_reg or force_done or start_addr_reg)
always @ (*)
begin: FSM_BEHAVIOR
    reset_ram          = 1'b0;
    from_process       = 1'b0;
    ...
    prev_start_addr    = prev_start_addr_reg;
    to_be_read         = to_be_read_reg;
    to_be_absorbed     = to_be_absorbed_reg;
    data_length        = data_length_reg;
    din_padded         = {`PARALLEL_SLICES{1'b0}};
    ...
    if (force_done) begin
        ...
        case(current_state)
            ...
            s_read_length: begin
                ...
                // extra pad control
                if((cshake_reg == 1'b0) && (din[`RATE_WIDTH-1:0] > (rate_reg-6))) begin
                    extra_pad = 1'b1;
                end else if((cshake_reg == 1'b1) && (din[`RATE_WIDTH-1:0] > (rate_reg-4))) begin
                    extra_pad = 1'b1;
                end else begin
                    extra_pad = 1'b0;
                end
                ...
                to_be_read = din[`RATE_WIDTH-1:0]; // counts down to 0
                to_be_absorbed = din[`RATE_WIDTH-1:0];
                data_length = din[`RATE_WIDTH-1:0]; // stores the value permanentlyt
            end
            ...
            s_absorb_data: begin 
                ...
                if((eof_internal_reg == 1'b1) && (to_be_absorbed_reg < `PARALLEL_SLICES) && (cshake_reg == 1'b1)) begin
                    ...
                    else if(`PARALLEL_SLICES > 1) begin
                        if(counter == (data_length_reg>> `CLOG2(`PARALLEL_SLICES))) 
                            din_padded[data_length_reg & (`PARALLEL_SLICES-1)] = 1'b0;
                        if(counter == ((data_length_reg+1)>> `CLOG2(`PARALLEL_SLICES))) 
                            din_padded[(data_length_reg+1) & (`PARALLEL_SLICES-1)] = 1'b0;
                ...
                else if((eof_internal_reg == 1'b1) && (to_be_absorbed_reg < `PARALLEL_SLICES) && (cshake_reg == 1'b0)) begin
                    if(`PARALLEL_SLICES == 1) begin
                        if(counter == data_length_reg) 
                            din_padded[0] = 1'b1;
                        else if(counter == (data_length_reg + 1))
            ...
            s_squeeze: begin
                ...
                else if((counter == max_reads_count_reg) || (requested_bytes_reg < `PARALLEL_SLICES)) begin       
                    counter_ctrl = 2'b00; // stopping from counter to become zero -- Sanjay
                    squeeze_output = 1'b1 & dout_valid; // check
                end
                else begin
