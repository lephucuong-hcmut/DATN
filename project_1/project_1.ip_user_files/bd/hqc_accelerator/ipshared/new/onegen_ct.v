`timescale 1ns / 1ps

module onegen_ct
    #(
    
    parameter M = 15,
    
    parameter E0_WIDTH = 32,
    parameter E0_DEPTH = 17696/32,
    parameter LOGE0W = `CLOG2(E0_DEPTH),
    parameter E0_FILE = "",
    
    parameter WIDTH = 32,
    parameter DEPTH = 17696/32,
    parameter LOGW = `CLOG2(DEPTH),
    parameter FILE = "",
    
    parameter WEIGHT   = 66,
    parameter LOG_WEIGHT   = `CLOG2(WEIGHT)
    
    )
    (
    input clk,
    input rst,
    input init_mem,
    input [(M-1):0] location,
    
    output reg [LOG_WEIGHT-1:0] rd_addr,
    output reg rd_en,
    input start,
    output collision,
    output ready,
    output valid,
    output done
    );
    
wire [WIDTH-1:0] gen_one;
wire [WIDTH-1:0] gen_one_reg_rev;
wire [WIDTH-1:0] data_0, data_1;
reg  [LOGW -1:0] addr_0, addr_1;
wire [LOGW -1:0] decode_addr;
wire [LOGW -1:0] addr_1_mux;
wire [LOGW -1:0] addr_read;
wire [WIDTH-1:0] q_0, q_1;
reg  wren_0, wren_1;



reg ready_s;
reg valid_s;
reg done_s;
reg init;
reg reading_out;

// Below are states
localparam s_wait_for_init_mem      =   0;//
localparam s_initialize             =   1;//
localparam s_init_done              =   2;//
localparam s_wait_start             =   3;//
localparam s_load_loc               =   4;//
localparam s_wait_last_2            =   5;//
localparam s_read_wait              =   6;//
localparam s_done                   =   7;//
localparam s_stall_for_ram          =   8;//
localparam s_stall_for_ram_2        =   9;//
localparam s_simulate_wait_start    =   10;//
localparam s_simulate_wait_start_last    =   11;//

reg [3:0] state     = 0;
reg [LOG_WEIGHT:0] count_reg;

reg [3:0] count_sim_cycles = 0;
assign ready = ready_s;
assign valid = valid_s;
assign done = done_s;



  genvar i;
  generate
    for (i = 0; i < WIDTH; i=i+1) begin:vector_gen
        assign gen_one[i] = (i == location % WIDTH) ? 1'b1 : 1'b0; 
    end
  endgenerate
  
//  always@(posedge clk)
//  begin
//    gen_one_reg = gen_one;
//  end 
  
  assign data_0 = 0;
  
  assign decode_addr = location/WIDTH;
  
  reg collision_s;
  
 //collision detection
 always@(posedge clk)
 begin
    if (init|start) begin
        collision_s <= 1'b0;
    end
    else if (wren_1) begin
        if (q_0 == data_1) begin
            collision_s <= 1'b1;
        end
    end
 end 

assign collision = collision_s;

assign data_1 = (q_0 | gen_one_reg_rev); 
  
  
  genvar j;
  generate
    for (j = 0; j < WIDTH; j=j+1) begin:data_inverse
        assign gen_one_reg_rev[j] = gen_one[WIDTH-j-1]; // fix
    end
  endgenerate
  


    mem_dual #(.WIDTH(WIDTH), .DEPTH(DEPTH), .FILE(FILE)) mem_dual_A (
    .clock(clk),
    .data_0(data_0),
    .data_1(data_1),
    .address_0(addr_read),
    .address_1(addr_1_mux),
    .wren_0(wren_0),
    .wren_1(wren_1),
    .q_0(q_0),
    .q_1(q_1)
  );
 

assign addr_read = reading_out? addr_0: decode_addr;

assign addr_1_mux = addr_1;

reg collision_reg;
reg collision_reg_reg;
//reg collision_reg_reg_reg;
//reg collision_reg_reg_reg_reg;
always@(posedge clk)
begin
    collision_reg <= collision;
    collision_reg_reg <= collision_reg;
//    collision_reg_reg_reg <= collision_reg_reg;
//    collision_reg_reg_reg_reg <= collision_reg_reg_reg;
end

always@(posedge clk)
begin
    if (rst) begin
        state <= s_wait_for_init_mem;
        addr_0 <= 0;
        count_reg <= 0;
        addr_1 <= 0;
        ready_s <= 1'b0;
        count_sim_cycles <= 0;
        
    end
    else begin
        if (state == s_wait_for_init_mem) begin
            done_s <= 1'b0;
            valid_s <= 1'b0;
            done_s <= 1'b0;
            addr_1 <= 0;
            addr_0 <= 0;
            rd_addr <= 0;
            count_reg <= 0;
            ready_s <= 1'b0;
            count_sim_cycles <= 0;
            if (init_mem) begin
                state <= s_initialize;
            end
            
        end
        
        else if (state == s_initialize) begin
            valid_s <= 1'b0;
            done_s <= 1'b0;
            addr_1 <= 0;
            addr_0 <= 0;
            rd_addr <= 0;
            count_reg <= 0;
            state <= s_init_done;
            ready_s <= 1'b0;
            
        end 
        
        else if (state == s_init_done) begin
            if (addr_0 == DEPTH - 1) begin
                addr_0 <= 0;
                state <= s_wait_start;
                ready_s <= 1'b1;
            end
            else begin
                addr_0 <= addr_0+1;
                state <= s_init_done;
            end
         end
         
         else if (state == s_wait_start) begin
            if (start) begin
                ready_s <= 1'b0;
                count_sim_cycles <= 0;
                if (rd_addr == WEIGHT-1) begin
                    
                    state <= s_wait_last_2;
                end 
                else begin
                    state <= s_load_loc;
                end
            end
         end
         
         else if (state == s_load_loc) begin
//            if (~collision_s) begin       
//                if (rd_addr == WEIGHT-1) begin
//                    state <= s_wait_last_2;
//                    addr_1 <= decode_addr;
//                    count_reg <= count_reg + 1;
//                end
//                else begin
//                    state <= s_stall_for_ram;
//                    rd_addr <= rd_addr + 1;
//                    addr_0 <= decode_addr;
//                    addr_1 <= decode_addr;
//                     count_reg <= count_reg + 1;
//                end
                state <= s_read_wait;      // CHUYỂN SANG CHỜ RAM ĐỌC
                addr_0 <= decode_addr;     // Cấp địa chỉ cho RAM
                addr_1 <= decode_addr;     // Chuẩn bị địa chỉ ghi
//            end
//            else begin
//                state <= s_wait_start;
//                rd_addr <= rd_addr - 1;
//                count_reg <= count_reg - 1;
//				ready_s <= 1'b1;
//            end  
         end
         
         // TRẠNG THÁI MỚI: Đợi 1 chu kỳ để q_0 từ BRAM trôi ra ổn định
         else if (state == s_read_wait) begin
            state <= s_stall_for_ram;
         end
         
         else if (state == s_stall_for_ram) begin
            if (~collision_s) begin  
//                if (collision_reg_reg) begin
//                    state <= s_load_loc;
//                end
//                else begin
//                    state <= s_simulate_wait_start;
//                end
                if (rd_addr == WEIGHT-1) begin
                    state <= s_wait_last_2;
                    count_reg <= count_reg + 1;
                end else begin
                    state <= s_simulate_wait_start;
                    rd_addr <= rd_addr + 1;     // CHUYỂN VIỆC TĂNG ĐỊA CHỈ XUỐNG ĐÂY
                    count_reg <= count_reg + 1;
                end
            end
            else begin 
                state <= s_wait_start;
                rd_addr <= rd_addr - 1;
                count_reg <= count_reg - 1;
				ready_s <= 1'b1;
            end
         end
         
         else if (state == s_simulate_wait_start) begin
            if (count_sim_cycles < 8) begin  
                state <= s_simulate_wait_start;
                count_sim_cycles <= count_sim_cycles + 1;
            end
            else begin
                state <= s_load_loc;
                count_sim_cycles <= 0;
            end
         end
         
         else if (state == s_wait_last_2) begin
            if (~collision_s) begin
                if (count_reg == WEIGHT+2-1) begin
                    count_reg <= 0;
                    if (collision_reg_reg) begin
                        state <= s_done;
                    end
                    else begin
                        state <= s_simulate_wait_start_last;
                    end
                    addr_0 <= 0;
                end
                else begin
                    state <= s_wait_last_2;
                    count_reg <= count_reg + 1;
                    addr_1 <= decode_addr;
                end
             end
            else begin 
                state = s_wait_start;
                count_reg <= count_reg - 1;
				ready_s <= 1'b1;
            end 
         end 
         
         else if (state == s_simulate_wait_start_last) begin
            if (count_sim_cycles < 8) begin  
                state <= s_simulate_wait_start_last;
                count_sim_cycles <= count_sim_cycles + 1;
            end
            else begin
                state <= s_done;
                count_sim_cycles <= 0;
            end
         end
             
              
        else if (state == s_done) begin
            done_s <= 1'b1;
            valid_s <= 1'b0;
            state <= s_wait_for_init_mem;
        end
    end 
  
end 

always@(*)
begin
    case (state)
      s_initialize: begin
                     wren_0 = 1'b1;
                     wren_1 = 1'b0;
                     init = 1'b1;
                     rd_en = 1'b0;
                     reading_out = 1'b1;
                    end
      
                   
      s_init_done: begin 
                    wren_0 = 1'b1;
                    wren_1 = 1'b0; 
                    init = 1'b0;
                    rd_en = 1'b0;
                    reading_out = 1'b1;
                   end  
      
      s_wait_start: begin
                        wren_0 = 1'b0;
                        wren_1 = 1'b0;
                        init = 1'b0;
                        reading_out = 1'b0;
                        if (start) begin
                            rd_en = 1'b1;
                        end
                        else begin
                            rd_en = 1'b0;
                        end
                    end  
                    
      s_load_loc: begin
                    rd_en = 1'b1;
                    wren_0 = 1'b0;
                    wren_1 = 1'b0;
                    init = 1'b0;
                    reading_out = 1'b0;
                  end
                  
      s_read_wait: begin
                    wren_0 = 1'b0;
                    wren_1 = 1'b0; // TẮT GHI KHI ĐANG ĐỢI
                    init = 1'b0;
                    rd_en = 1'b1;
                    reading_out = 1'b0;
      end
      
      s_stall_for_ram: begin
                            wren_0 = 1'b0;
                            wren_1 = 1'b1;
                            init = 1'b0;
                            rd_en = 1'b0;
                            reading_out = 1'b0;
                       end

      s_simulate_wait_start: begin
                                wren_0 = 1'b0;
                                wren_1 = 1'b0;
                                init = 1'b0;
                                rd_en = 1'b0;
                                reading_out = 1'b0;
                            end
                     
      s_wait_last_2: begin
                    wren_0 = 1'b0;
                    rd_en = 1'b0;
                    init = 1'b0;
                    reading_out = 1'b0;
                    if (count_reg < WEIGHT+1) begin
                        wren_1 = 1'b1;
                    end
                    else begin
                        wren_1 = 1'b0;
                    end
                  end 
      
      s_simulate_wait_start_last: begin
                                wren_0 = 1'b0;
                                wren_1 = 1'b0;
                                init = 1'b0;
                                rd_en = 1'b0;
                                reading_out = 1'b0;
                            end    
                  
      s_done:      begin 
                    wren_0 = 1'b0; 
                    wren_1 = 1'b0;
                    reading_out = 1'b0;
                    init = 1'b0;
                    rd_en = 1'b0;
                   end     
   
      default: begin
        wren_0 = 1'b0;
        wren_1 = 1'b0;
        init        = 1'b0;
        rd_en       = 1'b0;
        reading_out = 1'b0;
      end
    endcase

end     
endmodule